...
#---180 degree rotation matrix around the x axis because the camera and the tag's referential do not match
...
while True:
    ...
    if ids is not None:         
        ...
        for i in range(len(ids)):
            ...
            #-- print tag position in tag frame (minus on the y coordinator) MUDAR 
            aux = tvec[i][0].astype(float)
            
            #print(aux[0])
            str_pos = "i%dx%.2fy%.2fz%.2f" % (ids[i], aux[0], aux[1], aux[2])
            #print(str_pos)
            #if i==0:
                #coord='(0,50)'
            #if i==1:
            ...
            #if i==2:
                #coord='(0,150)'
            #cv2.putText(frame, str_position, coord, cv2.FONT_HERSHEY_PLAIN, 1, (0,255,0), 2, cv2.LINE_AA)
        
            #obtain rotation matrix tag->camera
            R_ct = np.matrix(cv2.Rodrigues(rvec[i][0])[0]) #transforms rotation vector to rotation matrix
            R_tc = R_ct.T
